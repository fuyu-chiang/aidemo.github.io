...
let img, imgred, imgblue, imgyellow, imgtext, imgstar, imgshining;
...
function preload(){
  ...
  imgshining = loadImage('asset/shining.png')
...
function draw() {
    ...
    if (Snoww == 1 && mouseIsPressed == true) {
        h=h+1;

        
        graphic.noStroke();
        graphic.ellipse(mouseX, mouseY, 20, 20);
    ...
    image(imgtext, -40,-50,width*.8,height*.8);
  ...
  //    }
    ...
    if(frameCount % 50 < 25)
    {
      image(imgshining, -width*.1,-height*0.3,width*1.8,height*1.8);
    }
